...
//			uint8_t sps[] = {0x00,0x00,0x01,0xb0,0x03,0x00,0x00,0x01,0xb5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,0x01,0x20,0x00,0xbc,0x04,0x06,0xc4,0x00,0x7b,0x0c,0xf0,0x12,0x1c,0x51,0x8f,0x00,0x00,0x01,0xb2,0x58,0x76,0x69,0x44,0x30,0x30,0x36,0x34};
            SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0", opaque->codecpar->extradata, opaque->codecpar->extradata_size);
//            free(convert_buffer);
        } else if(opaque->codecpar->codec_id ==AV_CODEC_ID_WMV3){
            // Codec specific data				
			uint8_t sps[] = {0x2d,0x00,0x00,0x00,0x80,0x07,0x00,0x00,0x38,0x04,0x00,0x00,0x01,0x00,0x18,0x00,0x57,0x4d,0x56,0x33,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
			size_t sps_size = 40+opaque->codecpar->extradata_size;
			uint8_t *convert_buffer = (uint8_t *)calloc(1, sps_size);
			memcpy(convert_buffer, sps, 40);
			memcpy(convert_buffer + 40, opaque->codecpar->extradata, opaque->codecpar->extradata_size);
            SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0", convert_buffer, sps_size);
			free(convert_buffer);
        } else {
			ALOGE("sps size=%d",opaque->codecpar->extradata_size);
			ALOGE("sps %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
			opaque->codecpar->extradata[0],
			opaque->codecpar->extradata[1],
			opaque->codecpar->extradata[2],
			opaque->codecpar->extradata[3],
			opaque->codecpar->extradata[4],
			opaque->codecpar->extradata[5],
			opaque->codecpar->extradata[6],
			opaque->codecpar->extradata[7],
			opaque->codecpar->extradata[8],
			opaque->codecpar->extradata[9],
			opaque->codecpar->extradata[10],
			opaque->codecpar->extradata[11],
			opaque->codecpar->extradata[12],
			opaque->codecpar->extradata[13],
			opaque->codecpar->extradata[14],
			opaque->codecpar->extradata[15]);
            // Codec specific data
...
static int feed_input_buffer2(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (d->pkt_temp.data) {
        ...
        if (input_buffer_index < 0) {
            ...
            if (!copy_size) {
                ALOGE("%s: SDL_AMediaCodec_getInputBuffer 21 failed\n", __func__);
                ret = -1;
                goto fail;
            }
        }
        ...
        if (amc_ret != SDL_AMEDIA_OK) {
            ALOGE("%s: SDL_AMediaCodec_getInputBuffer 22 failed\n", __func__);
            ret = -1;
            goto fail;
        }
...
static int feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (d->pkt_temp.data) {
        ...
        queue_flags = 0;
        input_buffer_index = SDL_AMediaCodec_dequeueInputBuffer(opaque->acodec, timeUs);
		ALOGI("raw %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x size=[%d],nal_size=[%d] ", 
            d->pkt_temp.data[0],
            d->pkt_temp.data[1],
            d->pkt_temp.data[2],
            d->pkt_temp.data[3],
        ...
        if (input_buffer_index < 0) {
            ...
            copy_size = SDL_AMediaCodec_writeInputData(opaque->acodec, input_buffer_index, d->pkt_temp.data, d->pkt_temp.size);
            if (!copy_size) {
                ALOGE("%s: SDL_AMediaCodec_getInputBuffer 11 failed\n", __func__);
                ret = -1;
                goto fail;
            }
        }
        ...
        amc_ret = SDL_AMediaCodec_queueInputBuffer(opaque->acodec, input_buffer_index, 0, copy_size, time_stamp, queue_flags);
        if (amc_ret != SDL_AMEDIA_OK) {
            ALOGE("%s: SDL_AMediaCodec_getInputBuffer 12 failed\n", __func__);
            ret = -1;
            goto fail;
        }
        // ALOGE("%s: queue %d/%d", __func__, (int)copy_size, (int)input_buffer_size);
...
//		goto fail;
        ...
        break;
	//@FlyZebra ATC8227L ADD
	case AV_CODEC_ID_H263:
        if (!ffp->mediacodec_all_videos) {
            ALOGE("%s: MediaCodec/AV_CODEC_ID_H263 is disabled. codec_id:%d \n", __func__, opaque->codecpar->codec_id);
            goto fail;
        }
        strcpy(opaque->mcc.mime_type, SDL_AMIME_VIDEO_H263);
        opaque->mcc.profile = opaque->codecpar->profile;
        opaque->mcc.level   = opaque->codecpar->level;
        break;
	//@FlyZebra ATC8227L ADD
...
//		goto fail;
        if (!ffp->mediacodec_all_videos) {
            ALOGE("%s: MediaCodec/AV_CODEC_ID_WMV3 is disabled. codec_id:%d \n", __func__, opaque->codecpar->codec_id);
            goto fail;
        }
        strcpy(opaque->mcc.mime_type, "video/x-ms-wmv");
        opaque->mcc.profile = opaque->codecpar->profile;
        opaque->mcc.level   = opaque->codecpar->level;
        break;
    default:
...
//		goto fail;
	...
	case AV_CODEC_ID_H263:
        ...
        opaque->mcc.profile = opaque->codecpar->profile;
        opaque->mcc.level   = opaque->codecpar->level;
		ALOGE("ADD AV_CODEC_ID_H263 FiNISH");
        break;
	//@FlyZebra ATC8227L ADD
	case AV_CODEC_ID_WMV3:
//		goto fail;
    ...
    jsurface = ffpipeline_get_surface_as_global_ref(env, pipeline);
	ALOGE("reconfigure_codec_l 4 width=%d,height=%d",opaque->codecpar->width,opaque->codecpar->height);
